            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-10">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="form-group-modern">
                  ...
                  <input type="number" step="0.01" value={form.amount} onChange={e => setForm({...form, amount: e.target.value})} className="auth-input w-full bg-surface-container-low focus:bg-surface-container-high border-none rounded-2xl p-5 font-display font-bold text-xl" required disabled={!canWrite} />
                </div>
                <div className="form-group-modern">
                  <label className="text-[0.65rem] uppercase tracking-[0.15em] font-black text-secondary/40 block mb-3">Settlement Date</label>
                  <input type="date" value={form.payment_date} onChange={e => setForm({...form, payment_date: e.target.value})} className="auth-input w-full bg-surface-container-low focus:bg-surface-container-high border-none rounded-2xl p-5 font-medium text-white" required disabled={!canWrite} />
                </div>
              </div>
              ...
              <div className="form-group-modern">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {['Cash', 'Bank Transfer', 'Online', 'Check'].map(m => (
                    <button key={m} type="button" onClick={() => setForm({...form, payment_method: m})} className={`py-4 rounded-2xl font-bold text-[0.7rem] uppercase tracking-widest transition-all ${form.payment_method === m ? 'bg-white text-on-primary scale-[1.02]' : 'bg-surface-container-low text-secondary/40'}`} disabled={!canWrite}>{m}</button>
                  ))}
                </div>
              </div>
              ...
              <div className="form-group-modern">
                ...
                <div className="flex gap-4">
                  {['Paid', 'Partial', 'Pending'].map(s => (
                    <button key={s} type="button" onClick={() => setForm({...form, status: s as any})} className={`flex-1 py-4 rounded-2xl font-bold text-[0.7rem] uppercase tracking-widest transition-all ${form.status === s ? 'bg-white text-on-primary scale-[1.02]' : 'bg-surface-container-low text-secondary/40'}`} disabled={!canWrite}>{s}</button>
                  ))}
                </div>
              </div>
