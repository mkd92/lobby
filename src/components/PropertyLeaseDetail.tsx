...
const PropertyLeaseDetail: React.FC = () => {
  ...
  const [form, setForm] = useState({
    ...
    start_date: '',
    end_date: '',
    status: 'Active' as PropertyLease['status'],
    notes: '',
  });
  ...
  const { data: lease, isLoading } = useQuery({
    queryKey: ['property-lease', id],
    queryFn: async () => {
      const snap = await getDoc(doc(db, 'property_leases', id!));
      if (!snap.exists()) throw new Error('Lease agreement not found');
      
      const data = { id: snap.id, ...snap.data() } as PropertyLease;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const leaseRef = doc(db, 'property_leases', id!);
      ...
      // If status changed to Expired/Terminated, free up the unit
      if (derivedStatus !== 'Active' && lease.unit_id) {
        batch.update(doc(db, 'units', lease.unit_id), { status: 'Vacant' });
      } else if (derivedStatus === 'Active' && lease.unit_id) {
        // If re-activated, mark unit as occupied
        batch.update(doc(db, 'units', lease.unit_id), { status: 'Occupied' });
      }

      await batch.commit();
      
      queryClient.invalidateQueries({ queryKey: ['property-lease', id] });
  ...
  const unitLabel = `Unit ${lease.unit_number}`;
  ...
  return (
    <div className="view-container page-fade-in">
      ...
      <header className="view-header flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div>
          <div className="view-eyebrow flex items-center gap-2 cursor-pointer group" onClick={() => navigate('/property-leases')}>
        ...
        <div className="lg:col-span-4 flex flex-col gap-8">
          <div className="glass-panel p-10 rounded-[40px] relative overflow-hidden">
            ...
            <div className="flex flex-col gap-8">
              <div>
                <label className="text-[0.6rem] uppercase tracking-[0.2em] font-black text-secondary/40 block mb-2">Legal Designation (Tenant)</label>
                <div className="text-white font-display font-bold text-2xl tracking-tight">{lease.tenant_name}</div>
              </div>
              <div>
                <label className="text-[0.6rem] uppercase tracking-[0.2em] font-black text-secondary/40 block mb-2">Portfolio Asset (Property)</label>
                <div className="text-white/80 font-medium text-lg">{lease.property_name}</div>
              </div>
              <div>
                <label className="text-[0.6rem] uppercase tracking-[0.2em] font-black text-secondary/40 block mb-2">Inventory Identification (Unit)</label>
          ...
          <div className="glass-panel p-10 rounded-[40px] bg-primary-container/5">
            ...
            <div className="flex flex-col gap-8">
              <div>
                <label className="text-[0.6rem] uppercase tracking-[0.2em] font-black text-secondary/40 block mb-2">Agreed Monthly Rent</label>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-10">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="form-group-modern">
                  <label className="text-[0.65rem] uppercase tracking-[0.15em] font-black text-secondary/40 block mb-3">Adjust Monthly Rent ({currencySymbol})</label>
                  <input 
                    type="number" 
                    step="0.01" 
                ...
                <div className="form-group-modern">
                  <label className="text-[0.65rem] uppercase tracking-[0.15em] font-black text-secondary/40 block mb-3">Target Expiry Date</label>
                  <input 
                    type="date" 
                    value={form.end_date} 
              ...
              {isOwner && (
                <footer className="flex flex-wrap justify-center sm:justify-end items-center gap-4 sm:gap-8 mt-6 pt-10 border-t border-white/5">
                  <button type="button" className="text-secondary/40 font-bold uppercase tracking-widest text-xs hover:text-white transition-colors" onClick={() => navigate('/property-leases')}>
                    Discard Changes
                  </button>
...
export default PropertyLeaseDetail;
