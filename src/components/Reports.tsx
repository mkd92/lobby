  ...
  const generatePDF = () => {
    ...
    const paymentRows = filteredOutstanding.map(p => {
      ...
      return `<tr>
        ...
        <td style="text-align:right">${sym}${Number(p.rent_amount).toLocaleString()}</td>
        <td style="text-align:right;color:#555">${sym}${Number(p.amount).toLocaleString()}</td>
        <td style="text-align:right;font-weight:800;color:#b91c1c">${sym}${balance.toLocaleString()}</td>
      </tr>`;
    }).join('');
    ...
    <div className="view-container" style={{ maxWidth: '860px', margin: '0 auto' }}>
      ...
      <div className="properties-metrics-bar" style={{ marginBottom: '2.5rem' }}>
        {[
          ...
          <div key={m.label} className="prop-metric" style={m.accent && m.value > 0 ? { background: 'rgba(185,28,28,0.08)' } : undefined}>
            <div className="prop-metric-label">{m.label}</div>
            <div className="prop-metric-value" style={m.accent && m.value > 0 ? { color: '#ef4444' } : undefined}>{m.value}</div>
          </div>
        ))}
      </div>
